...
class AddShopVC: UIViewController,
  ...
  func submitButtonSave(){
    ...
    database.setData(["\(imageFolderID)": [
      "shopNameTextField" :
        self.shopNameTextField.text!,
      "descriptionTextField" :
        self.descriptionTextField.text!,
      "phoneNumberTextField" :
        self.phoneNumberTextField.text!,
      "locationLinktextField" :
        self.locationLinktextField.text!,
      "typeShopTextField" :
        self.typeShopTextField.text!,
      "cityTextField" :
      ...
      db.collection("users")
        .document(auth!.uid)
        .setData(["hasStore":true], merge: true)
    }
    
    let logoImage = self.imagePhotoShop
    ...
    for image in imageData {
      ...
      storageRF
        .putData(image, metadata : uploadMetaData) { metadata, error in
          guard error == nil else {
            return
          }
          ...
          storageRF.downloadURL { url, error in
            if error != nil {
              ...
              db.collection("sections").document(type!).setData(
                ["\(imageFolderID)":["shopNameTextField" :
                                      self.shopNameTextField.text!,
                                     "descriptionTextField" :
                                      self.descriptionTextField.text!,
                                     "phoneNumberTextField" :
                ...
                db.collection("sections")
                  ...
                  .getDocument { snapshot, error in
                    if error != nil {
                      ...
                      self.shopNameTextField.text =
                        dataDoc["shopNameTextField"] as? String
                      self.imagePhotoShop
                        .sd_setImage(with: URL(string: dataDoc["logo"] as! String),
                                     placeholderImage: UIImage(named: ""))
                      self.descriptionTextField.text =
                        dataDoc["descriptionTextField"] as? String
                      self.phoneNumberTextField.text =
                        dataDoc["phoneNumberTextField"] as? String
                      self.locationLinktextField.text =
                        dataDoc["locationLinktextField"] as? String
                      self.typeShopTextField.text =
                        dataDoc["typeShopTextField"] as? String
                      self.cityTextField.text =
                        dataDoc["cityTextField"] as? String
                      
                      for image in dataDoc["images"] as! Array<String> {
                        let imageView = UIImageView()
                        imageView.sd_setImage(with: URL(string: image),
                                              placeholderImage: UIImage(named: "home"))
                        { imagee, error, cache, url in
  ...
  func doneSection() {
    ...
    let doneSectionButton = UIBarButtonItem(title: "Done",
                                            style: UIBarButtonItem.Style.plain,
                                            target: self,
                                            action: #selector(donePickerSection))
    
    toolBarSection.setItems([doneSectionButton], animated: false)
  ...
  @objc func donePickerSection() {
    typeShopTextField.text = sectionType[currentIndex]
  ...
  func doneCitys() {
    ...
    let doneCitysButton = UIBarButtonItem(title: "Done",
                                          style: UIBarButtonItem.Style.plain,
                                          target: self,
                                          action: #selector(donePickerCitys))
    
    toolBarCitys.setItems([doneCitysButton], animated: false)
...
extension AddShopVC: UIPickerViewDelegate,
  ...
  func pickerView(_ pickerView: UIPickerView,
    ...
    if (pickerView == pickerSection) {
      typeShopTextField.text = sectionType[row]
    } else {
...
extension AddShopVC:  UICollectionViewDelegate,
  ...
  func collectionView(_ collectionView: UICollectionView,
    ...
    cell.imagePhotos.image = photos[indexPath.row]
    return cell
  }
}
